...
# b/c of switching from Python 2.7 to 3+ these fns could potentially venture into the non-ascii realm,
# so I restrict everything to basic latin 0020 to 007F (that's 32-128 in decimal)

valid_chars = "".join([chr(i) for i in range(32, 127)])
valid_chars_len = len(valid_chars)
shift_amt = 1
shifted_chars = [valid_chars[(i + shift_amt) % valid_chars_len] for i in range(valid_chars_len)]


def encode_parameter(param, key=CIPHER_KEY):
    encoded_chars = []
    control = 0
    for p in param:
        if p not in valid_chars:
            # if the char is not valid, just skip it...
            encoded_chars.append(p)
        else:
            # wrap around the cipher key...
            control = 0 if control % len(key) == 0 else control
            # calculate the position C[p] = (M[p]+K[p]) mod len(M)
            result = (valid_chars.find(p) + shifted_chars.index(key[control])) % valid_chars_len
            # record the result...
            encoded_chars.append(shifted_chars[result])
            # increment the counter...
            control += 1
    encoded_parameter = "".join(encoded_chars)
    return encoded_parameter


def decode_parameter(param, key=CIPHER_KEY):
    decoded_chars = []
    control = 0
    for p in param:
        if p not in valid_chars:
            ...
            control = 0 if control % len(key) == 0 else control
            # calculate the position C[p] = (M[p]+K[p]) mod len(M)
            result = (shifted_chars.index(p) - shifted_chars.index(key[control])) % valid_chars_len
            # record the result...
            decoded_chars.append(valid_chars[result])
            # increment the counter...
            control += 1
    decoded_parameter = "".join(decoded_chars)
    return decoded_parameter

# simpler Python 2.7 way of doing things
...
def q_send_mail(mail_subject, mail_content, mail_from=settings.EMAIL_HOST_USER, mail_to=[settings.EMAIL_HOST_USER]):
    try:
...
# TODO: REMOVE DJANGUAR-SPECIFIC STUFF
...
class Version(object):
    ...
    def string_to_int(cls, string):
        ...
        number = 0
        total_bits = 0
        for n, b in reversed(list(zip(numbers, cls.N_BITS))):
            max_num = (b+1) - 1
            if n >= 1 << max_num:
